(function($) {
	...
	*/
	...
	function openCard(id) {
		...
		var menu_item = $('.main-menu a[href="' + id + '"]').closest('li');
		var menu_items = $('.main-menu li');
		var menu_item_home = $('.main-menu li.home');
		var card_item = $(id);

		if (menu_item.length === 0 || card_item.length === 0) return;
	...
	function getCurrentMenuItem() {
		return $('.main-menu li.active');
	}

	function openNextCard() {
		var current = getCurrentMenuItem();
	...
	// 监听滚轮：页内滚到底/滚到顶部时自动切换卡片
	$(document).on('wheel', '.card-inner', function(e) {
		...
		if (delta > 0 && scrollTop + height >= scrollHeight - 10) {
			openNextCard();
			return;
		}
